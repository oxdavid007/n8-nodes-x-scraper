...
import { CreateTwitterUserWithCookiesDto } from './base/CreateTwitterUserWithCookiesDto';
...
export class XScraperNode implements INodeType {
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const userData: CreateTwitterUserWithCookiesDto = {
			username: 'default_user', // You might want to make this configurable
		...
		try {
			...
			console.log('🚀 ~ XScraperNode ~ execute ~');
				...
				try {
					if (resource === 'tweet') {
						if (operation === 'create') {
							...
							const replyToTweetId = this.getNodeParameter('replyToTweetId', i) as
								| string
								| undefined;

							console.log('🚀 ~ XScraperNode ~ execute ~ replyToTweetId:', replyToTweetId);
							// Handle media if provided
							const mediaData = this.getNodeParameter('mediaData', i) as
								| {
										data: Uint8Array;
										mediaType: string;
								  }[]
